...
@section('content')
    <div class="section-header">
        ...
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ route('pelanggan.group') }}">Group</a></div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('body').on('click', '#group-chat', function() {
                ...
                channel.bind('chat', function (data) {
                    ...
                    if (groupID == data.message.group_id) {
                        $.get(url, function (chat) {
                            $.chatCtrl('#mychatbox', {
                                role: '{{ Auth::user()->role }}',
                                name: chat.name,
                                text: chat.text,
                                picture: chat.picture,
                                position: 'chat-' + chat.position,
                                time: chat.time,
                                type: 'text'
                            });
                        });
            ...
            $('#chat-form').on('submit', function(e) {
                ...
                var user_id = '{{ Auth::guard("pelanggan")->user()->id }}'
                $.ajax({
                    type: "POST",
                    url: '{{ url('pelanggan/group/chatadd') }}',
                    headers: {

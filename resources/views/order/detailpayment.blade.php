                                    ...
                                    @if ($order->payment <> '')
                                        ...
                                        @foreach($order->payment->where('order_id', $order->id)->get() as $key => $value)
                                            <tr>
                                                ...
                                                <td>{{ AllHelper::rupiah($value->nominal) }}</td>
                                                <td>
                                                    <a onclick="getPayment({{ $value->id }})" class="btn btn-primary text-white btn-sm mr-2 mb-2"><i class="fas fa-edit"></i> Edit</a>
                                                    @php
                                                        $url = "'".route('admin.order.payment.delete', $value->id)."'";
                                                    @endphp
                                                    <a onclick="deleteModal({{ $url }})" class="btn btn-danger text-white btn-sm mr-2 mb-2"><i class="fas fa-trash"></i> Hapus</a>
                                                </td>
                                            </tr>
                                        @endforeach
...
@section('modal')
    ...
    @include('order.partials.editpayment')
...
@section('script')
    <script>
        ...
        function getPayment(id) {
            var url = '{{ url("admin/order/get_payment") }}/' + id;
            $.ajax({
                url: url,
                type: 'GET',
                dataType: 'json',

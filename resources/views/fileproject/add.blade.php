...
@section('content')
    ...
    <div class="section-header">
        <div class="section-header-back">
            <a href="{{ route('admin.fileproject') }}" class="btn btn-icon"><i class="fas fa-arrow-left"></i></a>
        </div>
        <h1>File Project</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item"><a href="{{ route('admin.fileproject') }}">File Project</a></div>
                        ...
                        <div class="col-sm-12 col-md-7">
                            <select name="order" class="form-control select2">
                                ...
                                @foreach ($order as $row)
                                    <option value="{{ $row->id }}">{{ $row->kode_klien.' - '.$row->pelanggan->name.' - '.$row->user->name }}</option>
                                @endforeach
                            </select>
